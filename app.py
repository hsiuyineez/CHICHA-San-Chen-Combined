...
def account_information():
    ...
    # Retrieve points from shelve database
    with shelve.open('points_db') as points_db:
        points = points_db.get(user_email, 0)

    # Add points to user session
    user['points'] = points
...
def payment():
    if request.method == 'POST':
        ...
        user = session.get("user")  # Retrieve user info from the session
        if user:
            logger.info(f"User {user['email']} found in session")
            with shelve.open(DB_FILE, writeback=True) as db:
                db_user = db.get(user["email"])
                if db_user:
                    logger.info(f"User {user['email']} found in database")
                    db_user['points'] += int(total)  # Assuming points are 1 point per dollar spent
                    db[user["email"]] = db_user
                    session["user"] = db_user
                else:
...
def order_summary():
    ...
    # Calculate points (e.g., 1 point for every $1 spent)
    ...
    # Store points in shelve database
    with shelve.open(DB_FILE, writeback=True) as db:
        db_user = db.get(user_email)
        if db_user:
            db_user['points'] += points_earned
            db[user_email] = db_user
            session["user"] = db_user
        else:
